...
=============================
Dauerlauf-Version von matrix_sync_members.py: überwacht einen Space/Raum live
und lädt jede neue Person SOFORT in den Zielraum ein, statt erst beim
nächsten periodischen Lauf. Läuft dauerhaft im Hintergrund, wie
...
auf neue Beitritte zum Quellraum.
...
async def initial_sync_pass(client: AsyncClient, source_room: str, target_room: str, dry_run: bool) -> None:
    ...
    print(f"   {len(source_members)} Mitglieder gefunden. Prüfe Zielraum...\n")
...
async def watch(source_room: str, target_room: str, dry_run: bool) -> None:
    config = load_config()
    ...
    await initial_sync_pass(client, source_room, target_room, dry_run)
...
def main():
    ...
    print(f"👀 Überwache Quellraum: {args.source}")
